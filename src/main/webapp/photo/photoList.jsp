...
<%
   //이전 페이지 구현
   int beginRow = 0;
   int rowPerPage = 8; //  한 페이지에 내가 보고싶은 행 개수
...
<style>
   ...
   .containers{width:100%; margin:0 auto;overflow:hidden;text-align:center;}
   .gallery{margin:20px -20px;box-sizing:border-box;}
...
<body>
   ...
   <%-- <table class = "table-borderless container" style="clear:both;">
      <tr>
      <%
   ...
   <div class="containers">
      <div style="text-align:right; margin-right:88px;">
         <a href="<%=request.getContextPath()%>/photo/insertPhotoForm.jsp" class="btn btn-primary">이미지 등록</a>
      </div>
    <ul class="gallery">
	     <% 
	     	for(Photo p : list) { 
	     %>
	        <a href="<%=request.getContextPath()%>/photo/selectPhotoOne.jsp?photoNo=<%=p.getPhotoNo()%>">
	           <img src="<%=request.getContextPath()%>/upload/<%=p.getPhotoName()%>" width="230" height="300">
	        </a>
	     <%
	        } 
	     %>
